...
export class MembersListComponent implements OnInit, OnDestroy {
    ...
    get filteredUsers(): any[] {
        ...
        if (this.selectedCategories.length > 0) {
            filtered = filtered.filter((user) =>
                this.selectedCategories.includes(user.secteuractivite)
            );
        }
    ...
    ngOnInit(): void {
        this.authService.getRolePro().subscribe((data) => {
            this.usersPro = data;
        ...
        this.authService.getRoleProActif().subscribe((data) => {
            this.usersProActif = data;
            this.ActifCount = this.usersProActif.length; // Compter le nombre total d'utilisateurs
            console.log('les comptes actif' + this.ActifCount);
        });
        this.authService.getRoleProEnattente().subscribe((data) => {
            this.usersProEnattente = data;
            this.EnattenteCount = this.usersProEnattente.length; // Compter le nombre total d'utilisateurs
            console.log('les comptes en attente' + this.EnattenteCount);
        });
        this.authService.getRoleProRefuse().subscribe((data) => {
    ...
    getUserByCategorie(cat: string): number {
        const filteredUsers = this.usersPro.filter(
            (user) =>
                user.secteuractivite &&
                user.secteuractivite.trim().toLowerCase() ===
    ...
    getUsersProActifByCategory(cat: string): number {
    ...
    getUsersProEnAttenteByCategory(cat: string): number {
    ...
    getUsersProRefuseByCategory(cat: string): number {
    ...
    editUserRole(id: number) {
        ...
        this.router.navigate(['/ecommerce/order-details', id]);
    }

    editProRole(id: number) {
        // Naviguer vers le composant d'édition en passant l'ID du numéro d'urgence
        this.router.navigate(['/edit-pro-role', id]);
    }
}
