...
export class ListNotificationComponent implements OnInit, OnDestroy {
    ...
    ville: Ville[] = [];
    isToggled = false;
    fileName = 'liste-notification.xlsx';
    ...
    removeData(id: number) {
        if (window.confirm('Are sure you want to delete this Notification ?')) {
            this.notificationService.deleteData(id).subscribe(
                (data) => {
                    console.log(data);
    ...
    /*getNotificationByCategorie(cat: string): number {
        return this.usersPro.filter((user) => user.categories === cat).length;
    }*/
...
export class CreateUserDialogBox {
    ...
    ngOnInit() {
        ...
        this.notificationService.getnotification().subscribe((data) => {
            this.usersPro = data; 
            this.filteredNotifications = this.usersPro; 
            this.proCount = this.usersPro.length; 
        });
    }
    ...
    loadNotification() {
        this.notificationService.getnotification().subscribe((data) => {
            this.usersPro = data;
            console.log('Notification disponible :', this.usersPro);
        });
    }
    ...
    onSearch() {
        if (this.searchTerm) {
            ...
            this.filteredNotifications = this.usersPro.filter(
                (notification) =>
                    notification.titre
                        .toLowerCase()
                        .includes(this.searchTerm.toLowerCase()) ||
    ...
    filterNumUrgences() {
        ...
        if (this.selectedPayss.length > 0) {
            this.filteredNotifications = this.filteredNotifications.filter(
                (numUrgence) => this.selectedPayss.includes(numUrgence.pays)
            );
        }
        ...
        if (this.selectedCategories.length > 0) {
            this.filteredNotifications = this.filteredNotifications.filter(
                (numUrgence) =>
                    this.selectedCategories.includes(numUrgence.categorie)
            );
        }
    }
    ...
    filterNumUrgencesByPays() {
        if (this.selectedPayss.length > 0) {
            this.filteredNotifications = this.usersPro.filter((numUrgence) =>
